    ...
    <div className={`fixed left-0 top-0 h-full bg-background-secondary border-r border-border z-40 flex flex-col transition-all duration-300 ${
      ...
      <div className="p-4 border-b border-border flex items-center justify-between relative">
        ...
        <button
          ...
          className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 w-8 h-8 rounded-full bg-gradient-primary hover:opacity-90 active:scale-95 transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-xl z-10 group"
          title={isCollapsed ? 'Expand menu' : 'Collapse menu'}
        >
          <svg
            className={`w-4 h-4 text-background transition-transform duration-300 ${isCollapsed ? 'rotate-180' : ''}`}
            fill="none"
            stroke="currentColor"
            strokeWidth={2.5}
            viewBox="0 0 24 24"
